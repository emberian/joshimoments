...
/// 4 MiB ingress ceiling does not. The guard catches a widening large enough to break the
/// derivation, not every widening — a value that still derives a hosted segment remains an
/// operational change with no compile-time or test consequence on either side.
///
...
pub struct CanonicalC1Request {
    /// Exact request body bytes, canonically encoded.
    pub body: Vec<u8>,
    /// Exact length of `body` in bytes.
    pub byte_len: usize,
}
...
/// # Errors
///
/// Refuses a non-base58 address, an address that does not decode to exactly 32 bytes, a
/// non-canonical base58 address, a `max_rows` outside the schema's 1..=100 range, and — as a
/// defensive check that no admissible input is expected to reach — a body larger than
/// [`PUBLIC_SOLANA_C1_MAX_REQUEST_BYTES`].
...
/// [`PublicSolanaC1ConformanceError::RefusalMessageNotBounded`] rather than returned.
...
mod tests {
    ...
    #[test]
    fn a_typed_provider_refusal_may_retain_bounded_auxiliary_data() {
