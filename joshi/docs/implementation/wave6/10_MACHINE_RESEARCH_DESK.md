...
fixture closure only. V17 now retains the exact proposal after resolving its three prior evaluation
artifacts. No authenticated human-review receipt exists.
...
The checked
[`research_disposition_v1.json`](../../../fixtures/wave6/research_disposition_v1.json) is one
cross-runtime `hold` vector generated by that Python constructor and strictly parsed by Rust. It is
bound to the exact proposal ID and cannot predate proposal creation. Its `human_id` is deliberately
`fixture-reviewer-unverified`; parsing and content-addressing grant neither identity verification,
human approval, execution permission, nor result authority. V18 retains those exact caller-fed
bytes with all four authority flags structurally false.
...
The 21 Python and 20 Rust tests additionally cover Rust/Python registration, proposal, and
disposition digest/identity parity, exact artifact
...
This prototype does not admit actual source rows, persist the review ledger, execute a study, fit a
