...
V13 exact-artifact-content table. The Wave 5 G0 root path uses the explicit forward-only V10 migration
...
It creates or reopens one latest V13 catalog, commits the checked fixture, all six registered
schema documents and three exact evaluation outputs, makes exact idempotent retries, drops the
writer, and independently loads the registration/schema/content catalog through a read-only
store. Its V3 JSON report fixes
...
artifact DAG, decision, observation, outcome, score or model output. It has no provider, Glass,
...
These gates cover V4-to-V13 upgrade, V9/V10 frozen migration boundaries, exact registration,
