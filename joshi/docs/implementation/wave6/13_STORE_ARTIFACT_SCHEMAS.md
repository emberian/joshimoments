...
Status: **PASS for durable exact schema bytes under N00; V13 retains four evaluation contents and
...
program row to the seven checked schema documents already named by that registration.
...
The seven retained documents are:

| Kind | Schema |
| --- | --- |
| `campaign_registration_fixture` | `joshi.wave6.campaign-registration.v1` |
| `domain_known_truth_evaluation_fixture` | `joshi.analysis.wave6-domain-known-truth/v1` |
...
The Core `wave6-program-registration` command commits all seven in this exact order. Its V5 report
emits each kind/schema/digest/commit tuple, requires `registeredSchemaCount="7"`, then proves the
same tuples and bytes through a read-only reopen. Re-running the command yields the same seven
...
The four checked evaluation artifacts under `fixtures/wave6/artifacts` exactly implement their
...
The store suite commits, retries and read-only reopens all seven exact schema files and exercises the
