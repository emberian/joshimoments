...
Migration V13 adds `wave6_fixture_artifact_content_v1`. The sole store accepts the four checked
domain, generic, protocol and structural known-truth evaluation outputs only after their N00
program and V12 schema rows exist.
...
3. revalidates fixed authority, source-fixture digest(s), exact 7/8/7/3 result denominator and
...
Core's V6 registration report commits, exactly retries and read-only reopens all four artifacts
after the seven schemas, then separately commits their exact V14 fixture DAG. The foundation witness
