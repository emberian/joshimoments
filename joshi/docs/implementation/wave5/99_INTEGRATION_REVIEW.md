...
| Cockpit publication -> operator | `apps/glass` | Strict bounded client contracts, stable accepted order, private pending-command/act transport, exact ACK rules and evidence-only controls | **intrinsic contract** / fixture UI | Core mounts neither ordinary exchange nor cockpit index/open; Glass consumes its own operational contract rather than Cockpit V2. No attached-browser screen-reader, keyboard/focus, zoom/reflow, contrast or pointer/touch witness qualifies accessibility or product use. |
| One-time code -> scoped session | `joshi-pairing` | Single-use/rate/expiry/revoke state machine; test entropy/clock stay test-only; production boundary requires a clock; secrets are zeroized, nonserializing and redacted | **intrinsic contract** | It is memory-only and unmounted. More importantly, Rust requires a 64-lowercase-hex code and returns origin/epoch/monotonic-ms metadata with capability out-of-band, while Glass accepts human codes such as `EMBER-482901` and expects UTC expiry/scopes/capability in one response. No reviewed translation or monotonic-to-wall bridge exists. |
| Pairing/client -> core routes | `apps/core::service` | Legacy loopback snapshot/operator routes and fail-closed prospective stubs; static capability file is owner-only/redacted | **durable offline** for legacy scene/operator rows only | No ordinary pairing exchange, W5 status, Cockpit V2 index/open, export or import route. Prospective launch/nomination/abstention remain unavailable. |
