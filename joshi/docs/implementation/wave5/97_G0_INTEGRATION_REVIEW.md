...
all 37 required rows. Literal OS-process termination covers all mapped boundaries and literal Rust
panic now covers all twelve scheduled panic rows, but the twelve scheduled power-loss rows and a
real product presentation remain absent. Neither partial ledger raises the fault/root or product
gate, and no Boolean readiness claim may say otherwise.
