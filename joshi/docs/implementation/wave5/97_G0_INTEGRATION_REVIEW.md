...
| Fault matrix | Crash/retry at reserve, I/O, spool, store commit, publication prepare/body/head, pairing consume, memory ACK, export/import, backup/restore | **PASS for strict false contract, 37-row in-process ledger, and actual process kill at all 36 mapped boundaries; BLOCKED qualifying mixed-mode root result** | Core now parks a separate child inside each exact mapped fault check, kills it, and recovers that same fresh state. The complete process-kill ledger observed all 36 boundaries: 33 closed the eighteen-role root bundle and three retained typed, hashed refusals at the exact supervisor boundaries where a finite attempt was cancelled or provider evidence was not durably associated. A first run exposed interrupted original-root quarantine around restored reopen; bounded idempotent quarantine recovery repaired both reopen rows. Twelve frozen rows requested process kill and match exactly. The other 24 request power loss or panic, which an OS process kill does not prove, so `mixedScheduledModesFullyExecuted` and `fullOfflineFaultWalk` remain false. |
