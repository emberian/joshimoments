...
| Fault matrix | Crash/retry at reserve, I/O, spool, store commit, publication prepare/body/head, pairing consume, memory ACK, export/import, backup/restore | **PASS only for strict false contract + partial baseline mapping; BLOCKED full executable/root** | The source/publication/memory/status implementation passes fourteen exact before/after prefix interruptions. The root-workspace harness now maps eleven exact baseline artifacts to `observed_partial`, but reservation fault injection and the other required steps are incomplete and it does not execute all 37 scenarios. |
| Root evidence bundle | Canonical machine-readable evidence with exact IDs, digests, cutoffs, clocks, paths/keys, receipts and negative qualification bits, independently reverified after reopen | **PASS only as an eleven-role partial artifact bundle; BLOCKED root** | The component report binds reservation, origin, receipt, binding, ACK, fact, prepare, head, act, episode, and degraded-status identities/digests, with one exact role per observed step. Paired-read fault evidence, export/import, positive status recovery, backup/restore and final root reopen remain absent; both qualification bits stay false. |
...
eleven artifact digests rederived from its owner/store objects; the harness maps those roles
