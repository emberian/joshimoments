...
origin. The root readiness wrapper now runs the component and paired-route/restart/scripted-
presentation smoke through one join, exact-matches their run/source/publication lineage, and binds
both nested report digests, the eighteen-role evidence-bundle digest, and V10 snapshot ID into a
schema-V5 witness. The scripted claim and its pairing state live in a separately restored V21
overlay; the component truth stays V10. The witness
remains a partial
...
| Pairing + Glass | OS entropy; trusted origin and time; bounded rate/expiry; one-time durable consume/revoke/restart; same-origin open of exact publication | **PASS only as isolated durable paired-publication API + Glass inspection/report protocol; BLOCKED default-product/root G0** | The SQLite exchange opens the exact headed offline-fixture publication only for the declared scopes; wrong scope, revoke, and restart refuse. Root readiness runs a finite in-process exchange/open, stores and exact-retries one explicitly scripted V21 presentation claim, restarts the epoch, refuses the old capability, and verifies both the byte-identical open and store claim from a distinct restored latest V22 overlay. The explicit Glass inspector independently parses the V2 closure and submits the same bounded claim without action sinks. No browser instance was connected for UI QA, `browserPresented` remains false, and no product presentation occurrence or complete root fault bundle exists. |
...
| Backup/restore | Backup includes SQLite plus every referenced artifact/CAS/publication/origin object; restore into a distinct location; artifact-bearing reads and digest revalidation | **PASS as one complete offline-fixture baseline recovery set; BLOCKED fault/root** | Store-owned writers separately back up the immutable V10 component catalog and mutable latest V22 pairing/presentation overlay with their reachable artifacts; the root join also exact-copies the supervisor journal, health, identity, origin and ACK files except the process lock. It restores all three, makes both original catalogs plus blob/export/supervisor roots unavailable, and reopens the exact run/source/publication/memory/export/import/status/pairing/presentation/origin solely from restored roots. Six before/after composite retry cases pass, but this remains baseline recovery rather than the full qualifying schedule. |
| Fault matrix | Crash/retry at reserve, I/O, spool, store commit, publication prepare/body/head, pairing consume, memory ACK, export/import, backup/restore | **PASS for strict false contract, 37-row in-process ledger, actual process kill at all 36 mapped boundaries, and actual panic at all 12 scheduled panic rows; BLOCKED power-loss/product root result** | Core parks a separate child inside each exact mapped fault check for OS-kill evidence and recovers that same fresh state. The complete process-kill ledger observed all 36 boundaries: 33 closed the eighteen-role root bundle and three retained typed, hashed refusals at exact supervisor boundaries. A distinct child path now fsyncs the marker and executes a real Rust panic only for each of the twelve scheduled panic rows; all twelve close complete same-state eighteen-role bundles, and a non-panic row is rejected. The twelve power-loss rows remain unexecuted as power loss, no browser presentation exists, and neither partial ledger sets `mixedScheduledModesFullyExecuted` or `fullOfflineFaultWalk`. |
| Root evidence bundle | Canonical machine-readable evidence with exact IDs, digests, cutoffs, clocks, paths/keys, receipts and negative qualification bits, independently reverified after reopen | **PASS for one eighteen-role baseline plus a 37-row nonqualifying diagnostic ledger; BLOCKED fault-root** | `wave5-g0-root-evidence.v2` runs the component and pairing/open/scripted-presentation/restart smoke across the V10 truth and latest V22 overlay, exact-matches run registration, source occurrence, publication/head and claim/receipt/store digests, and extends the fifteen component artifacts with consumed pairing, exact HTTP read and a final dual-store+supervisor restored-root readback. The presentation remains nested evidence rather than a new harness role. The schema-V5 readiness witness binds that baseline; the separate deep ledger keeps process-loss qualification false and typed refusals intact. |
...
- That combined result initially predated the backup reservation/snapshot edits to migration 0010,
  ...
  `sha256:47fd336ae56b2f3f9acebd5198b096d96f36b54b6cf46e50564762d405a33ce9`,
  snapshot `sha256:acaed49221b40e4dbd81f99f9c7c24ef4e96414179424d8415921e0c86f8ae6f`
  and manifest `sha256:45ba8f6abcca8d30fe5f141060d6618bfc79badad1d5bdcfb8af3e56f53eec2b`.
...
export/import/status/backup, paired API read, scripted presentation store/retry, and dual-catalog
restored-root reopen now form one eighteen-role baseline evidence bundle with nested presentation
evidence. The deep ledger emits exact diagnostic results across all 37 required rows. Literal
OS-process termination covers all mapped boundaries and literal Rust panic covers all twelve
scheduled panic rows, but the twelve scheduled power-loss rows and a real attached/product
presentation remain absent. Neither partial ledger raises the fault/root or product gate, and no
Boolean readiness claim may say otherwise.
